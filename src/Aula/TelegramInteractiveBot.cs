...
public class TelegramInteractiveBot
{
    ...
    private async Task HandleAulaQuestion(long chatId, string text, bool isEnglish)
    {
        try
        {
            ...
            if (isEnglish)
            {
                await SendMessage(chatId, "I'm not sure how to help with that. You can ask me about a child's activities like 'What is TestChild2 doing tomorrow?' or 'Does TestChild1 have homework for Tuesday?'");
            }
            else
            {
                await SendMessage(chatId, "Jeg er ikke sikker på, hvordan jeg kan hjælpe med det. Du kan spørge mig om et barns aktiviteter som 'Hvad skal TestChild2 lave i morgen?' eller 'Har TestChild1 lektier for til tirsdag?'");
            }
        }
        catch (Exception ex)
        {
